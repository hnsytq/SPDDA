...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    if dataset_name == 'Houston13':
        ...
        rest_band = 108
        ignored_labels = [0]
    elif dataset_name == 'whuS012':
        img = open_file(folder + 'HSI_S1_0012.tif')
        gt = open_file(folder + 'Label_S1_0012.mat')['label']
        rgb_bands = [20, 30, 30]
        if gt.shape[0] == 1:
        ...
        gt[gt == 16] = 5
        gt[gt == 17] = 6
        label_values = ['Paddy field', 'Dry farm', "Woodland", 'Urban built-up',
                        'Rural settlement', "Other construction land"]
        ignored_labels = [0]
    elif dataset_name == 'whuT060':
        img = open_file(folder + 'HSI_T1_0060.tif')
        gt = open_file(folder + 'Label_T1_0060.mat')['label']
        rgb_bands = [20, 30, 30]
        if gt.shape[0] == 1:
            gt = np.squeeze(gt, axis=0)
        gt[gt == 15] = 4
